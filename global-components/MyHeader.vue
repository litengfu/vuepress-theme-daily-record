<template>
  <el-header id="topHeader" :style="{paddingLeft: headerLeft + 'px'}" class="top-header"
    :class="{headerShadow: hasShadow}">
    <el-row type="flex" align="middle" class="header-warp">
      <el-col :span="12" :xs="{span: 7}">
        <el-row type="flex" align="middle">
          <el-col :span="2">
            <el-button @click="clickMenu" type="primary" circle key="menusBtn">
        ...
        <div class="grid-content bg-purple-light">
          <el-row type="flex" align="middle" justify="end">
            <el-col :span="13" :xs="{span: 24}" :sm="{span: 21}" :md="{span: 17}" :lg="{span: 13}">
              <el-autocomplete v-model="searchVal" @select="handleSelect" class="search-input"
                popper-class="search-popper" :fetch-suggestions="querySearch" :placeholder="placeholder" size="small"
                :trigger-on-focus="false" clearable @focus="getSearch">
                <i slot="suffix" class="el-input__icon el-icon-search search-ico"></i>
                <template slot-scope="{ item }">
                  <div class="name">{{item.title}}</div>
                  <span class="addr addr-active"
                    v-if="hasResults">{{ item.strippedContent.slice(item.searchIndex, item.searchIndex + queryStrlen) }}</span>
                  <span class="addr addr-last"
                    v-if="hasResults">{{ item.strippedContent.slice(item.searchIndex + queryStrlen) }}</span>
                </template>
              </el-autocomplete>
            </el-col>
          </el-row>
