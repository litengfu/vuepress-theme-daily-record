<template>
  <div :key="$page.title">
    <el-row type="flex" justify="center" class="post-content">
      <el-col :span="16" :xs="{ span: 24 }" :sm="{ span: 23 }" :md="{ span: 23 }" :lg="{ span: 16 }"
        class="post-card post-sign" id="post-card">
        <Content></Content>
      ...
      <el-col :span="6" class="post-toc" id="post-toc" :class="{ 'open-toc': hasToc }">
        ...
        <div class="catalog-body">
          <ul id="catalog-list" class="catalog-list">
            <li v-for="(item, index) in catalogList" :key="index" class="toc-li"
              :class="{ active: currentIndex === index }">
              <a class="toc-link ellipsis" :href="'#' + item.id" :style="{ marginLeft: offsetList[index] * 12 + 'px' }">{{
                'H' +
                (offsetList[index] + 1) +
                ' . ' +
    ...
    <el-row type="flex" justify="space-around" class="post-nav">
      <el-col :span="7" class="post-prev">
        <div v-if="!isNaN(prevPost)">
          ...
          <router-link tag="p" :to="content[prevPost].path || '/'" class="nav-title">{{ content[prevPost].title }}
          </router-link>
        </div>
      </el-col>
      <el-col class="post-next" :lg="{ pull: 5 }" :span="7">
        <div v-if="!isNaN(nextPost)">
          ...
          <router-link tag="p" :to="content[nextPost].path || '/'" class="nav-title">{{ content[nextPost].title }}
          </router-link>
        </div>
      </el-col>
    </el-row>
