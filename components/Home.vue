  ...
  <div class="marginTop">
        ...
        <el-card class="box-card">
          ...
          <div v-if="(item.excerpt && item.tags.length)">
            ...
            <div class="post-footer" v-if="item.tags.length">
              <el-tag @click="toTaglist" v-for="(tag, index) in item.tags" :key="index" size="medium" :hit="true">
                {{tag}}</el-tag>
            </div>
          </div>
        </el-card>
    ...
    <el-row class="pagination-wrap" type="flex" justify="center" align="middle" v-show="content.length">
      <el-col :span="16" :xs="{span: 24}" :sm="{span: 20}" :md="{span: 18}" :lg="{span: 16}">
        <el-pagination :current-page="currentPage" class="pagination" background @current-change="changePage"
          layout="prev, pager, next" :page-size="pagination" :total="content.length" key="fenyei"></el-pagination>
      </el-col>
    </el-row>
  </div>
</template>
